...
import com.ducnh.ibatis.datasource.DataSourceFactory;
...
import com.ducnh.ibatis.mapping.DatabaseIdProvider;
...
public class XMLConfigBuilder extends BaseBuilder{
		...
		for (Object key : props.keySet()) {
			if (!metaConfig.hasSetter(String.valueOf(key))) {
				throw new BuilderException(
					"The setting " + key + " is not known. Make sure you spelled it correctlt (case insentive).");
			}
		}
		return props;
	}
	...
	private void typeAliasesElement(XNode context) {
		...
		for (XNode child : context.getChildren()) {
			if ("package".equals(child.getName())) {
				...
				configuration.getTypeAliasRegistry().registryAliases(typeAliasPackage);
			} else {
	...
	private void settingsElement(Properties props) {
		...
		configuration.setLazyLoadingEnabed(booleanValueOf(props.getProperty("lazyLoadingEnabled"), false));
		configuration.setAggressiveLazyLoading(booleanValueOf(props.getProperty("aggresiveLazyLoading"), false));
		configuration.setUseColumnLabel(booleanValueOf(props.getProperty("useColumnLabel"), true));
		configuration.setUseGeneratedKeys(booleanValueOf(props.getProperty("useGeneratedKeys"), false));
		configuration.setDefaultExecutorType(ExecutorType.valueOf(props.getProperty("defaultExecutorType", "SIMPLE")));
		configuration.setDefaultStatementTimeout(integerValueOf(props.getProperty("defaultStatementTimeout"), null));
		configuration.setDefaultFetchSize(integerValueOf(props.getProperty("defaultFetchSize"), null));
		configuration.setDefautlResultSetType(resolveResultSetType(props.getProperty("defaultResultSetType")));
		...
		configuration.setDefaultEnumTypeHandler(resolveClass(props.getProperty("defautlEnumTypeHandler")));
		configuration.setCallSettersOnNulls(booleanValueOf(props.getProperty("callSettersOnNulls"), false));
		configuration.setUseActualParamName(booleanValueOf(props.getProperty("useActualParamName"), true));
		configuration.setReturnInstanceForEmptyRow(booleanValueOf(props.getProperty("returnInstanceForEmptyRow"), false));
		configuration.setLogPrefix(props.getProperty("logPrefix"));
		configuration.setConfigurationFactory(resolveClass(props.getProperty("configurationFactory")));
		configuration.setShrinkWhitespacesInSql(booleanValueOf(props.getProperty("shrinkWhitespcaesInSql"), false));
			...
			if ("package".equals(child.getName())) {
				...
				if (resource != null && url == null && mapperClass == null) {
					...
					try (InputStream is = Resources.getResourceAsStream(resource)) {
						XMLMapperBuilder mapperParser = new XMLMapperBuilder(is, configuration, resource,
								configuration.getSqlFragments());
						mapperParser.parse();
					}
				} else if (resource == null && url != null && mapperClass == null) {
					ErrorContext.instance().resource(url);
					try (InputStream is = Resources.getUrlAsStream(url)) {
						XMLMapperBuilder mapperParser = new XMLMapperBuilder(is, configuration, url,
								configuration.getSqlFragments());
						mapperParser.parse();
					}
				} else if (resource == null && url == null && mapperClass != null) {
